...
primes_list = list_primes(number_to_calc_for_factors * 100) # arbritary number usually the largest factor of our number is less than 100 times the size of the number
...
for i in range(0, len(exps_list)): # iterate through each exponent of the number we want or the prime factors of the number of factors, here is 500
    if exps_list[i] > 2: # if the exponent is greater than 2, we know that it must be assoicated with the next smallest prime number
        ...
        for combo in list_combos: # for each perm in the list of perms
            print("\n")
            subtotal =1
            for nums in range(0, len(combo)): # for each num in this perm
...
print("CHECKING THIS NUMBER:",count_factors(new_total),"FACTORS", "| IS TRIANGULAR NUMBER:", is_triangular_number(new_total))
